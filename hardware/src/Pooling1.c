 ...
 */
// first_layer                    0
// node                           <dory.Parsers.HW_node.HW_node object at 0x7fc2226faeb0>
// sdk                            gap_sdk
...
void Pooling1(
 ...
 uint8_t *im2col;
  ...
  for(iter=0; iter<1*3*2; iter++) {
    ...
    x_tile_size_nif = (last_nif) ? 4 : 4;
    x_tile_size_h   = (last_h)   ? 4 : 78;
    x_tile_size_w   = (last_w)   ? 66 : 94;
    x_tile_size_byte = x_tile_size_nif*x_tile_size_h*x_tile_size_w*8/8;
    x_length_nif_byte = (last_nif)   ? 4 : 4;
    ...
    DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 78, 94, 4, 160, 4,  0, 0,0, pad_offset_h, pad_offset_w, 0, 8);
    ...
    y_tile_size_h   = (last_h)   ? 2 : 39;
    y_tile_size_w   = (last_w)   ? 33 : 47;

    x = (uint8_t *) (l1_buffer + 0);
    y = (uint8_t *) (l1_buffer + 29336);


    y_tile_size_nof = (last_nof) ? 4 : 4;
    y_tile_size_h   = (last_h)   ? 2 : 39;
    y_tile_size_w   = (last_w)   ? 33 : 47;
    y_tile_size_byte = y_tile_size_nof*y_tile_size_h*y_tile_size_w*8/8;
    y_length_nof_byte = (last_nof)   ? 4 : 4;
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
...
// aggiungere padding su tutti i lati, acc_out, and filter asymettric
    ...
    DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 39, 47, 4, 80, 4, 0, 0, 0, 0, 0, 0, 8);
    DMA_copy_y.loc = (l1_buffer + 29336);
