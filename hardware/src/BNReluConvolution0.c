...
void BNReluConvolution0(
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      x_tile_size_h   = (_i_h_load+1 == 4)   ? 33 : 97;
      x_tile_size_w   = (_i_w_load+1 == 2)   ? 133 : 189;
      ...
      if(_i_w_load > 0)
        pad_offset_w = 1;
      y_tile_size_h   = (_i_h_load+1 == 4)   ? 16 : 48;
      y_tile_size_w   = (_i_w_load+1 == 2)   ? 66 : 94;
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 97, 189, 1, 320, 1,  1, 1,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 4, 3*3, 1, 3*3, 1, 0,0,0,0,0,0, 8);
        DMA_copy_W.loc = (l1_buffer + 36397);
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+36 + 16*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 36441;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+52 + 16*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 36465;
    ...
    k = (int32_t *) (l1_buffer + 36441);
    lambda = (int32_t *) (l1_buffer + 36465);
    W = (uint8_t *) (l1_buffer + 36397);
    y = (uint8_t *) (l1_buffer + 18341);
    p_r = 0;
    p_l = 0;
    p_t = 0;
    p_b = 0;
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 48, 94, 4, 160, 4, 0, 0, 0, 0, 0, 0, 8);
      DMA_copy_y.loc = (l1_buffer + 18341);
