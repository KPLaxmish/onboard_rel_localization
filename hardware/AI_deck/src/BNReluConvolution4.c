...
void BNReluConvolution4(
  ...
  for(iter=0; iter < total_tiles; iter++) {
    // check if last in any dimension
      ...
      {
        DMA_copy_x.ext = dory_get_tile_3d(l2_x, _i_h_load, _i_w_load, _i_nif_load, 40, 40, 8, 40, 8,  2, 2,0, pad_offset_h, pad_offset_w, 0, 8);
      ...
      {
        DMA_copy_W.ext = dory_get_tile_3d(l2_W, _i_nof_load, 0, _i_nif_load, 2, 3*3, 8, 3*3, 8, 0,0,0,0,0,0, 8);
        ...
        DMA_copy_k.ext = (uint32_t) l2_W+144 + 8*_i_nof_load;
        DMA_copy_k.loc = (uint32_t) l1_buffer + 16168;
        DMA_copy_k.length_1d_copy = (uint16_t) W_tile_size_nof * 4;
        dory_dma_memcpy_async(&DMA_copy_k);
        dory_dma_barrier(&DMA_copy_k);

        DMA_copy_lambda.ext = (uint32_t) l2_W+152 + 8*_i_nof_load;
        DMA_copy_lambda.loc = (uint32_t) l1_buffer + 16184;
    ...
    k = (int32_t *) (l1_buffer + 16168);
    lambda = (int32_t *) (l1_buffer + 16184);
    ...
    pi_cl_team_barrier(0);
      DMA_copy_y.ext = dory_get_tile_3d(l2_y, _i_h_load, _i_w_load, _i_nof_load, 40, 40, 2, 40, 2, 0, 0, 0, 0, 0, 0, 8);
